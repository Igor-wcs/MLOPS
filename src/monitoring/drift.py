...
def obter_modelo_producao():
    ...
    try:
        ...
        if not versoes:
            logger.warning("Nenhum modelo encontrado no Registry. O primeiro treino ainda não ocorreu.")
            return None
...
def gerar_relatorio_drift(ticker="PETR4.SA", window_size=30):
    ...
    try:
        ...
        logger.warning("Ativando Fallback: Gerando dados sintéticos para garantir a continuidade da DAG.")
        
        datas_mock = pd.date_range(end=date.today(), periods=500)
        dados_mock = pd.DataFrame({
            'Close': np.linspace(32, 38, 500) + np.random.randn(500)
        }, index=datas_mock)
    ...
    if modelo is not None:
        try:
            with torch.no_grad():
                logger.info("Gerando predições com o modelo Champion para análise de desvio...")
                preds_ref = modelo(torch.tensor(X_ref_np, dtype=torch.float32)).numpy()
                preds_curr = modelo(torch.tensor(X_curr_np, dtype=torch.float32)).numpy()

            df_ref["predicao_modelo"] = preds_ref
            df_curr["predicao_modelo"] = preds_curr
        except Exception as e:
            logger.error(f"Erro ao gerar predições: {e}. Prosseguindo apenas com análise de features.")
    else:
        logger.info("Pulando análise de predições. O relatório focarão apenas no Drift das Features (preços).")
