...
class TestAgentTools:
    ...
    def test_tools_have_descriptions(self) -> None:
        ...
        for tool in tools:
            ...
            assert len(tool.description) > 15, f"A ferramenta '{tool.name}' tem uma descrição muito curta."
    ...
    def test_tools_contain_required_domain_tools(self) -> None:
        ...
        has_rag = any("conhecimento" in name.lower() or "rag" in name.lower() or "documento" in name.lower() for name in names)
...
class TestToolExecution:
    ...
    def test_stock_lookup_mocked(self, mock_ticker) -> None:
        ...
        mock_ticker.return_value.info = {"currentPrice": 38.50, "shortName": "PETROBRAS"}
    ...
    def test_invalid_ticker_handling(self) -> None:
        ...
        from src.agent.tools import _stock_lookup # Assumindo que a função base se chama assim
        
        result = _stock_lookup("TICKER_FALSO_123")
        assert "erro" in result.lower() or "não encontrado" in result.lower() or "sem dados" in result.lower()