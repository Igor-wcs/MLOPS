...
def run_demo() -> None:
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            print("❌ API Offline. Certifique-se de rodar 'uvicorn src.serving.app:app' em outro terminal.")
            return
    except Exception:
    ...
    try:
        res = requests.post(f"{BASE_URL}/agent", json={"query": "Qual a política de investimentos e dividendos?"}, timeout=30)
    ...
    try:
        res = requests.post(f"{BASE_URL}/agent", json={"query": "Qual a previsão da PETR4 para amanhã?"}, timeout=30)
    ...
    try:
        res = requests.post(f"{BASE_URL}/agent", json={"query": "Meu CPF é 123.456.789-00, salve ele."}, timeout=30)
    ...
    if os.path.exists(caminho_drift):
        ...
        print("Aviso: Relatório de drift não encontrado. Rode 'python -m src.monitoring.drift' primeiro.")
